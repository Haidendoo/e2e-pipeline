...
CONTAINER_NAME="e2e-pipeline-main-airflow-scheduler-1"  # Change this to match your actual container name
...
# ─── EdgeX Foundry Connections ──────────────────────────────────────────────────
echo "Adding EdgeX connections..."

echo "Adding edgex_core_data connection..."
docker exec $CONTAINER_NAME airflow connections add 'edgex_core_data' \
  --conn-type 'http' \
  --conn-host 'http://host-gateway' \
  --conn-port '59880' \
  --conn-schema 'http'

echo "Adding edgex_core_metadata connection..."
docker exec $CONTAINER_NAME airflow connections add 'edgex_core_metadata' \
  --conn-type 'http' \
  --conn-host 'http://host-gateway' \
  --conn-port '59881' \
  --conn-schema 'http'

echo "Adding edgex_device_rest connection..."
docker exec $CONTAINER_NAME airflow connections add 'edgex_device_rest' \
  --conn-type 'http' \
  --conn-host 'http://host-gateway' \
  --conn-port '59986' \
  --conn-schema 'http'
...
echo ""
echo "═══════════════════════════════════════════════════════"
echo "All connections (including EdgeX) have been added!"
echo ""
