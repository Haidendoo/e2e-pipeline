...
INFLUX_BUCKET = os.getenv("INFLUX_BUCKET_VM")
IP = os.getenv("IP")
...
def _print_log2_hist_copy(vals, val_type, strip_leading_zero):
    ...
    if idx_max <= 32:
        # header = "usecs:count"
        body = f"cpudist,ip={IP},a=%s,bucket=%d-%d count=%d {timestamp}"
        stars = stars_max
    else:
        # header = "usecs:count"
        body = f"cpudist,ip={IP},a=%s,bucket=%d-%d count=%d {timestamp}"
    ...
    for i in range(1, idx_max + 1):
        ...
        if (low == high):
            low -= 1
        val = vals[i]
        bucket_mid = i-1  # coordinate
        a = chr(ord('a') + bucket_mid)

        if strip_leading_zero:
            if val:
                # print(body % (low, high, val))
                point = body % (a, low, high, val)
                # print(point)
    ...
    def print_log2_hist(self, val_type="value", section_header="Bucket ptr",
        ...
        if isinstance(self.Key(), ct.Structure):
            ...
            for bucket in buckets:
                ...
                _print_log2_hist_copy(vals, val_type, strip_leading_zero)
        else:
            vals = [0] * log2_index_max
            for k, v in self.items():
                vals[k.value] = v.value
